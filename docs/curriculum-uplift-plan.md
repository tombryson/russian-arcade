...
**Status:** implementation in progress, reviewed 23 September 2026. This update describes the working `codex/curriculum-priorities` branch based on `5932faf`. Engineering checks and browser coverage are recorded in the implementation guide and [PR #3](https://github.com/tombryson/russian-arcade/pull/3). Native audio playback still needs a clean browser pass; no new course release or gate is enabled.

Shared contracts and release routing now support four authored A1 units, generated reading/listening practice and narrow Translation/Word Jumble observations. A reproducible human-review packet is prepared. All new results remain diagnostic; no qualified human or learner validation is claimed. See [implementation status](curriculum-implementation.md) and [package progress](#161-current-progress) for the current code and remaining acceptance criteria.
...
The [generated inventory](curriculum-coverage.md) lists all 239 reference requirements and 110 legacy targets. The crosswalk has one equivalent, 76 partial, 17 related and 16 unmapped links. There are now 48 directly authored task definitions across 18 A1 requirements: 22 contextual choices, 16 typed forms, three prepared listening items, four Writing tasks and three narrow Speaking diagnostics. These are task and definition counts, not completed requirements.
...
| --- | --- | --- |
| CU-01 | Complete reference/legacy inventory, versioned mapping and generated gap report, including four authored units. | Qualified review of the provisional mapping and source interpretation. Keep new task definitions in the generated inventory. |
...
| CU-05 | Four classified A1 units with contextual choices, typed forms and distinct Writing tasks. The original location unit retains its prepared listening stage and directions link. | Qualified language and learner review. New units still need suitable listening media; existing Speaking links are not unit-bound transfer assessments. |
| CU-06 | Generated A1–B2 reading and audio-first listening freeze source-based criteria before answers. New tasks record playback, transcript, word-translation and hint receipts; new question versions inherit support. | A clean native-audio browser playback check, recording quality and human review of question validity and marking. Client playback receipts do not prove attention or independent listening. |
| CU-07 | Sixteen authored typed-form prompts, plus new Translation tasks with one explicit language focus. A2–B2 Word Jumble maps only its visible connective demand; unrestricted A1 and legacy games remain unscoped. | Human comparison of accepted alternatives, ambiguity and whether a generated source elicits the claimed feature. The provider is instructed to leave an unobserved feature unscored; tests alone cannot establish that it does so reliably. |
| CU-08 | New A1–B2 Writing contracts and exact-response criterion reports, saved atomically with existing feedback. Each authored unit now has its own communicative criterion. | Human comparison across levels, source-based task adequacy and evidence that level demands are appropriate. Earlier live integration calls did not validate marking quality. |
| CU-09 | Three A1 Fluent directions situations carry one original-audio location-question criterion. Missing or uncertain speech can remain unscored. | Real recordings with known grammatical errors, independent acoustic/linguistic comparison and broader reviewed scenario coverage. Current technical tests use synthetic PCM and mocked judgements. |
| CU-10 | Four original A1 units now cover selected place, possession, recipient and routine contrasts. The vocabulary pipeline is retained and the gap inventory is explicit. | Most required A1 teaching, a reviewed lexical plan, sufficient task variation and content review. Eighteen linked requirements are not eighteen completed requirements. |
| CU-11 | Individual task contracts, support receipts and saved results provide reusable foundations. | A frozen five-domain assessment blueprint, component lifecycle, resumable submissions and comparable retries. The units are not that assessment. |
| CU-12 | Validation workflow and a hash-pinned 66-case reviewer packet, with provisional author keys separate from blank review fields. | Qualified independent marking, real speech material, learner pilots, adjudication and standard-setting. No human review is recorded and no new threshold is justified. |
| CU-13 | The units use the existing task player and return links. Curriculum/Journey separation and release-aware navigation remain. | Five-domain Profile summaries, new-release switch preview and recovery flows once the assessment exists; learner testing on desktop and mobile. |
...
Migrations `048`–`053` add release identities, shared task/report records, authored and generated listening support, Comprehension question/attempt history, and scoped sentence-production reports. Translation also retains reference-disclosure receipts. Existing published catalogue bytes, vocabulary, card schedules, rewards and course passes are retained. See the [implementation guide](curriculum-implementation.md) for storage details. Migration/import rehearsals and the offline coverage and reviewer checks pass. Browser checks cover the new units and transcript-supported Comprehension, including later question versions. Full regression CI is tracked in PR #3. The in-app browser crashed when its native audio control was pressed, so successful playback is not claimed. These safeguards do not replace linguistic validation.

### 16.2 Next implementation priorities

1. **Finish playback verification (CU-06).** The current pass covers interrupted requests, profile changes, disclosure before scoring, receipt inheritance and missing/replaced recordings in automated tests. Transcript disclosure and supported answer saving also work in the browser. Repeat native audio playback in a stable browser and verify the completed-playback receipt before closing this item. Preserve ordinary practice when a diagnostic cannot be supported.
2. **Review the content and marking (CU-05, CU-08, CU-09, CU-12).** Use the [reviewer packet](curriculum-validation.md) to obtain independent language and rubric judgements. Resolve ambiguous keys and legitimate alternatives before treating the tasks as an assessment bank. Add consented audio with known case/conjugation errors; compare the original audio, transcription and judgement.
3. **Expand A1 coverage (CU-10).** The place, possession/absence, object/recipient and routine units are now authored. Prepare and review missing audio where it serves the task, then select the next teaching targets from the gap inventory. Continue using existing vocabulary, form and media pipelines. Do not turn illustrative examples into a claim of complete lexical coverage.
4. **Evaluate generated tasks across levels (CU-06–09).** Compare provider judgements with adjudicated human labels. Check whether question sources actually elicit their criteria, whether alternatives are accepted, and whether supported attempts remain marked as supported. Track false passes, false failures, invalid reports and abstentions separately.
5. **Assemble the five-domain pilot (CU-11–14).** Once content and adapters are adequate, add the frozen blueprint and component workflow. Human comparison and a documented pass policy must precede a new access gate.

This pass adds provider-free tests and fixtures for generated listening, support inheritance, narrow sentence criteria, authored units and reviewer exports. They test storage, ownership, report structure and declared answer rules. They do not measure learning outcomes, establish natural audio quality or supply human sign-off. No new paid model evaluation is included in the pass.
