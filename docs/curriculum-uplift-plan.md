...
**Status:** implementation in progress, reviewed 23 September 2026. This update describes the working `codex/curriculum-priorities` branch based on `5932faf`. Final combined tests and browser verification are pending; no new course release or gate is enabled.
...
Migrations `048`–`053` add release identities, shared task/report records, authored and generated listening support, Comprehension question/attempt history, and scoped sentence-production reports. Translation also retains reference-disclosure receipts. Existing published catalogue bytes, vocabulary, card schedules, rewards and course passes are retained. See the [implementation guide](curriculum-implementation.md) for storage details. Final combined regression and migration checks for this working pass are pending; these safeguards do not replace linguistic validation.

### 16.2 Next implementation priorities

1. **Finish the current technical pass (CU-02, CU-06, CU-07).** Run the combined regressions and inspect reading, listening, support disclosures, typed forms and sentence feedback in a browser. Check interrupted requests, profile changes, disclosure before scoring and missing/replaced recordings. Preserve ordinary practice when a diagnostic cannot be supported.
