...
Shared contracts and release routing now support seven authored A1 units, generated reading/listening practice and narrow Translation/Word Jumble observations. A resumable five-domain diagnostic pilot is implemented; its two listening recordings are pending provider allowance. Review exports cover the expanded material. All new results remain diagnostic; no qualified human or learner validation is claimed. See [implementation status](curriculum-implementation.md) and [package progress](#161-current-progress) for the current code and remaining acceptance criteria.
...
The [generated inventory](curriculum-coverage.md) lists all 239 reference requirements and 110 legacy targets. The crosswalk has one equivalent, 76 partial, 17 related and 16 unmapped links. There are now 84 directly authored teaching-task definitions across 23 A1 requirements: 40 contextual choices, 28 typed forms, six prepared listening items, seven Writing tasks and three narrow Speaking diagnostics. The separate assessment pilot has two forms in each of five domains. These are task and definition counts, not completed requirements.
...
| --- | --- | --- |
| CU-01 | Complete reference/legacy inventory, versioned mapping and generated gap report, including seven authored units. | Review the provisional mapping and source interpretation; keep new task definitions in the generated inventory. |
...
| CU-05 | Seven classified A1 units with contextual choices, typed forms and distinct Writing tasks. Location and possession have complete listening packs. | Finish pending recordings; evaluate content and transfer tasks. Existing Speaking links are not unit-bound transfer assessments. |
...
| CU-10 | Seven A1 units cover selected place, possession, recipient, routine, agreement, pronoun and basic-motion contrasts. Existing vocabulary pipelines remain intact. | Remaining A1 teaching, lexical planning, variation and content review. Twenty-three linked requirements are not twenty-three completed requirements. |
| CU-11 | Frozen five-domain blueprint; owned drafts, original text/audio, separate results, review recovery and selected-component retakes. Existing task providers and budgets are reused. | Two pilot recordings remain pending. Evaluate marking and alternate-form comparability with real consenting learners. No overall pass is awarded. |
| CU-12 | Versioned review exports, 120 authored unit cases, separate pilot material, fingerprinted review ingestion and disagreement handling. Internal review corrected ambiguous language and criteria. | Optional external content review; real speech material, learner pilots and standard-setting before a future proficiency gate. Model review is not recorded as human review. |
...
Migrations `048`–`054` add release identities, shared evidence, listening support, Comprehension history, sentence-production reports and six pilot tables. Existing published catalogue bytes, vocabulary, card schedules, rewards and course passes are retained. See the [implementation guide](curriculum-implementation.md) for storage and verification details. A fresh browser run resolved the earlier playback uncertainty: native playback, replay, speed selection and answer saving worked for authored listening; generated Comprehension playback and reload also passed with a provider-free fixture. These checks do not establish pronunciation or grading quality.
...
5. **Design the next progression release (CU-14).** Use pilot findings to justify its pass and retake policy. A diagnostic sample must not silently become an A1 unlock gate. Preserve existing passes and open practice.

This pass adds tests for expanded units, bounded audio preparation, pilot ownership, draft conflicts, media preservation, review recovery, imports and review exports. They test implementation and declared answer rules. They do not measure learning outcomes, establish natural audio quality or substitute for an evaluation dataset.
