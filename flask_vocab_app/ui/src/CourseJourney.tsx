...
export function CourseJourney({chapterId,attemptId,language='en',progression}:{chapterId?:string;attemptId?:string;language?:Language;progression:ProgressionState}) {
  ...
  return <section class={`page course-page${attemptId ? ' is-checkpoint' : ''}`}>
    ...
    {!loading && visibleCourse && (attemptId ? visibleAttempt && <>
      ...
      <header class="course-heading"><p class="kicker">{visibleCourse.band} · {position(chapter.number)} · {stateText(chapter.status)}</p><h1>{t(chapter.title,chapter.title_ru)}</h1><p>{t(chapter.intro,chapter.intro_ru)}</p>{visibleCourse.release_id!=='a1-journey-v2' && !!chapter.objectives?.length && <ul class="course-objectives">{chapter.objectives.map(item=><li key={item.en}>{t(item.en,item.ru)}</li>)}</ul>}</header>
      ...
      <ol class="course-chapters">{visibleCourse.chapters.map(item=>item.status==='locked' ?
        <li class="course-chapter is-locked" key={item.id} aria-label={t(`Milestone ${item.number}, locked`,`Этап ${item.number}, пока недоступен`)}><span class="course-chapter-number" aria-hidden="true">{item.number}</span></li> :
        <li class={`course-chapter is-${item.status}`} key={item.id}><span class="course-chapter-number" aria-hidden="true">{item.status==='passed' ? '✓' : item.number}</span><div><p class="kicker">{position(item.number)} · {stateText(item.status)}</p><h2><a href={`#journey/chapter/${encodeURIComponent(item.id)}`}>{t(item.title,item.title_ru)}</a></h2><p>{t(item.intro,item.intro_ru)}</p><div class="course-chapter-progress"><progress max={1} value={clamp(item.progress)} aria-label={t(`Chapter ${item.number} progress`,`Прогресс главы ${item.number}`)} /><span>{Math.round(clamp(item.progress)*100)}%</span></div></div></li>)}</ol>
