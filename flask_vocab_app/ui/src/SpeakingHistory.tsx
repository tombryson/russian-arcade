...
type HistoryItem = SavedConversation & { kind: 'live' | 'recorded' };

/** Read both stores without moving records or starting any speech processing. */
    ...
    void Promise.allSettled([
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          next.push(...result.value.sessions.filter(session => index === 0 || session.mode === 'conversation')
            .map(session => ({ ...session, kind: index === 0 ? 'live' as const : 'recorded' as const })));
        }
      });
    ...
    {open && <>
      {loading ? <p class="quiet" role="status">{t('Loading conversations…', 'Загружаем разговоры…')}</p> : <>
        ...
        {!!items.length && <ul>{items.map(item => <li key={`${item.kind}:${item.id}`}>
          <a href={item.kind === 'live' ? `#speaking/${item.id}` : `#speaking/recorded/${item.id}`}>
